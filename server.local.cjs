// server.local.cjs — Local & Production API (combined)
...
function readDb() {
  try {
    ...
    return { tickets: {}, payments: [], emails: [], credits: {}, submissions: [], users: {}, raffle: { history: [] } };
  }
}
...
function pickSpotlightWinner(db) {
  const subs = db.submissions?.length
    ? db.submissions
    : [
...
app.get('/api/spotlight/current', (_req, res) => {
  ...
  const payload = spot
    ? { date: spot.date, nextSelectionUTC: spot.nextSelectionUTC || next10amISTasUTCDate().toISOString(), winner: spot.winner, creditsGranted: spot.creditsGranted }
    : { date: new Date().toISOString().slice(0, 10), nextSelectionUTC: next10amISTasUTCDate().toISOString(), winner: { id: 'u3', name: 'Rahul S', points: 780, title: 'Edge caching patterns for SPA' }, creditsGranted: 100 };
  res.json(payload);
});
